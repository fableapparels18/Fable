  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (res.ok) {
            toast({
                title: 'OTP Sent!',
                description: 'We have sent an OTP. Please check the console.',
            });
            setStage('otp');
        } else {
    ...
    <div className="flex min-h-[80vh] items-center justify-center bg-background px-4">
      <Card className="w-full max-w-sm">
        <CardHeader>
          ...
          <CardDescription>
            {stage === 'details' ? 'Join the FableFront family and start your story.' : 'Enter the OTP we sent to your console.'}
          </CardDescription>
        </CardHeader>
        {stage === 'details' ? (
             ...
             <form onSubmit={handleCreateAccount}>
                <CardContent className="grid gap-4">
                     <div className="grid gap-2">
                        ...
                        <Input id="otp" type="text" inputMode="numeric" pattern="\d{6}" placeholder="6-digit OTP" required value={otp} onChange={(e) => setOtp(e.target.value)} disabled={isLoading} />
                    </div>
                </CardContent>
                <CardFooter className="flex flex-col gap-4">
                    <Button className="w-full" type="submit" disabled={isLoading}>
